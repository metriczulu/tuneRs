...
class GridTest(unittest.TestCase):
    ...
    def test_resampled(self):
        ...
        test_resampled = GridSearchResample(LogisticRegression(random_state=0), {'C': [0.1, 1, 10]}, random_state=1)
    ...
    def test_split(self):
        from tuneRs.tuneRs import GridSearchResample
        from sklearn.linear_model import LogisticRegression
        test_split = GridSearchResample(LogisticRegression(random_state=0), {'C': [0.1, 1, 10]},
                                        val_data=(self.x_test, self.y_test), random_state=1)
        test_split.fit(self.x_train, self.y_train)
        test_split_score = int(test_split.best_score_ * 10)  # should be 6
        test_split_params = int(test_split.best_params_['C'] * 10)  # should be 1
